...
- **Judge prefers sandbox screenshots**: Iteration judging now reuses `agent_last_screenshot.png` captured inside the sandbox and only falls back to a host screenshot when the sandbox image failed to provide one. This removes the benchmark workflow's host-side Playwright, Shiny, and R package installs.
